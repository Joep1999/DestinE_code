...
def pre_process_destinE_data(files, timestep_interval, timesteps, date_str, date, knmi_input_dir, destineE_datafolder, local_folder_today, historical_destine, radar_xr):
    ...
    if date.hour>12:
        ...
        print('DestinE data combined and loaded successfully')
    else:
        destinE_data =  xr.open_dataset(files)
        print('DestinE data loaded successfully')
    destinE_data_np = destinE_data.tp.values
    #TODo make this neat
    param = '218.228-219.228-228.128'
    destinE_data['tp'].attrs = {'long_name': 'Accumulated precipitation', 'units': 'mm/h', 'param': '193.1.0'}
    ...
    for i in range(precipitation.shape[0]):
        destinE_data_radar_scale.append(rainfarm.downscale(precipitation[i], ds_factor=4, kernel_type = 'gaussian'))
    ...
    # Write to netcdf so cdo can use the data
    cdo_to_netcdf(date, destinE_data, precipitation, destinE_nlgrid_hres_advected, destineE_datafolder,  local_folder_today + f'pre-processed/DestinE_ExtremesDT_{date_str}_{param}_hres_advect_xr_{timestep_interval}_{timesteps}.nc', f"{timestep_interval}min", historical_destine)

    # REGRID DESTINE DATA TO KNMI RADAR GRID
    cdo.remapnn(
        f"{knmi_input_dir}knmi_grid.txt",                  # target grid
        input=local_folder_today + f'pre-processed/DestinE_ExtremesDT_{date_str}_{param}_hres_advect_xr_{timestep_interval}_{timesteps}.nc',    # source file
        output= local_folder_today + f'pre-processed/DestinE_ExtremesDT_{date_str}_{param}_hres_interp_nlgrid_{timestep_interval}_{timesteps}.nc'         # output file
    )

    #Open created grid
    destinE_nlgrid = xr.open_dataset(local_folder_today + f'pre-processed/DestinE_ExtremesDT_{date_str}_{param}_hres_interp_nlgrid_{timestep_interval}_{timesteps}.nc')
...
def run_blending_operational(date,historical_destine, knmi_input_dir, destineE_datafolder,timesteps, timestep_interval, n_ens_members,n_ens_members_dgmr, weights_method, custom_weights = None, return_weights = False, re_do_blending = False, multi_model = True, noise = True, probmatching = True, pysteps_nowcast = False, custom_extention = None, machine_learning_weights = True):
    ...
    try:
        if gauge_adjusted:
            fns = io.find_by_date(
                date_5min, knmi_input_dir, "%Y/%m/%d", "RAD_NL25_RAC_RT_%Y%m%d%H%M", "h5", 5, num_prev_files=3
            )
            assert len(fns[0]) == 4, f'fns does not contain enough radar images for DGMR (needs 4, contains {len(fns[0])})'

        else:
            fns = io.find_by_date(
                date_5min, knmi_input_dir, "%Y/%m/%d", "RAD_NL25_PCP_NA_%Y%m%d%H%M", "h5", 5, num_prev_files=3
            )
            assert len(fns[0]) == 4, f'fns does not contain enough radar images for DGMR (needs 4, contains {len(fns[0])})'
        ...
        fns = download_radar_knmi(gauge_adjusted,last_hour_5min,date_5min, knmi_input_dir)
    ...
    #pre-processing DestiNE
    #Make sure local folder exists
    local_folder_today = destineE_datafolder + 'ExtremesDT/{}/{}/'.format(yr,str(mnth).zfill(2))
    file_pre_processed = local_folder_today + f'pre-processed/DestinE_ExtremesDT_{date_str_day}_218.228-219.228-228.128_hres_interp_nlgrid_{timestep_interval}_{timesteps}.nc'
    file_original = local_folder_today + f'original/DestinE_ExtremesDT_{date_str_day}_218.228-219.228-228.128.grib'

    if os.path.exists(file_pre_processed):
        try:
            param = '218.228-219.228-228.128'
            extention = 'grib'
            destinE_nlgrid = xr.open_dataset(file_pre_processed)
            #slice the timesteps to match the radar timesteps
            ...
            param = '218.228-219.228-228.128'
            extention = 'grib'
            destinE_nlgrid_blend = pre_process_destinE_data(file_original, timestep_interval, timesteps, date_str_day, date, knmi_input_dir, destineE_datafolder, local_folder_today, historical_destine, R_xr)

    elif os.path.exists(file_original):
        param = '218.228-219.228-228.128'
        extention = 'grib'
        destinE_nlgrid_blend = pre_process_destinE_data(file_original, timestep_interval, timesteps, date_str_day, date, knmi_input_dir, destineE_datafolder, local_folder_today, historical_destine, R_xr)
    else:
    ...
    if multi_model == True:
        ...
        hour = date.hour
        forecast_hour = round_down(hour)
        local_folder_today_ifs = destineE_datafolder + 'IFS/{}/{}/'.format(yr,str(mnth).zfill(2))
        file_pre_processed_ifs = local_folder_today_ifs + f'pre-processed/IFS_{date_str_day}{forecast_hour}_hres_interp_nlgrid_{timestep_interval}_{timesteps}.nc'
        file_original_ifs = local_folder_today_ifs + f'original/IFS_{date_str_day}{forecast_hour}_228.128.grib'

        if os.path.exists(file_pre_processed_ifs):
            param = '218.228-219.228-228.128'
            extention = 'grib'
            IFS_nlgrid = xr.open_dataset(file_pre_processed_ifs)
        ...
        elif os.path.exists(file_original_ifs):
            param = '218.228-219.228-228.128'
            extention = 'grib'
            IFS_nlgrid_blend = pre_process_IFS_data(date, timestep_interval, timesteps, knmi_input_dir,destineE_datafolder, historical_destine, R_xr)
        else:
            print('ERROR: no pre-processed or original IFS file found')
        #Merge EXTREMSDT and IFS and make sure it is in correct format for blending.
        
        IFS_nlgrid_blend_swap = IFS_nlgrid_blend.transpose('ensemble', "time", "y", "x")
        destinE_nlgrid_blend_coord_ens = destinE_nlgrid_blend.assign_coords(coords = {'ensemble' : 51.0})
        destinE_nlgrid_blend_ens = destinE_nlgrid_blend_coord_ens.expand_dims(dim = 'ensemble', axis = 0)
        IFS_ExtremesDT_blend = xr.concat([destinE_nlgrid_blend_ens, IFS_nlgrid_blend_swap],dim = 'ensemble')
    ...
    assert len(DGMR_det[0]) == (timesteps + 1), f'length of DGMR output is not the same as the timesteps value, len is {len(DGMR_det[0])}!'
    ...
    if not os.path.exists(path_blend) or re_do_blending is True:
        ...
        if n_replaced > 0:
            print(f"Replacing {n_replaced} non-finite values in precip_cascades with 0.")
        # try:
        if pysteps_nowcast:
    ...
    # except: #If the blending fails, it is likely due to a an error with x= non-finite number in Gamma determination. Use climatological weights instead.
        #     print('Error in blending with weights method:', weights_method, ' - switching to custom climatological weights')
        #     GAMMA = np.array([
                #     [0.99805, 0.9933],
                #     [0.9925,  0.9752],
                #     [0.9776, 0.923],
        ...
        if return_weights:
            ...
            np.save(path_blend_weights, weights, allow_pickle=True)
        ...
        if return_weights:
            try:
                weights = np.load(path_blend_weights, allow_pickle=True)
            except:
                weights = []
