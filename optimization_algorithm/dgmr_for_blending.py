...
def run_dgmr(R, module=module):
    ...
    prediction_2 = predict(
        module, prediction_1[-4:], include_input_frames_in_result=False
    )
    prediction_2 = np.reshape(prediction_2, (18, 1536, 1280, 1))

    prediction_3 = predict(
        module, prediction_2[-4:], include_input_frames_in_result=False
    )
    prediction_3 = np.reshape(prediction_3, (18, 1536, 1280, 1))

    prediction_4 = predict(
        module, prediction_3[-4:], include_input_frames_in_result=False
    )
...
def run_dgmr_ensemble(R, ens_members, module=module):
    ...
    with ThreadPoolExecutor(max_workers=ens_members) as executor:
        futures = [executor.submit(run_dgmr, R, module) for _ in range(ens_members)]
